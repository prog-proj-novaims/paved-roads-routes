...
As main limitations from this application is that it only identify closest nodes considering linear distances and the centroid from the origin polygons. It is also not considered the distance from the origin polygons to the node that the route starts. In the future the sql function can be improved in order to consider origin polygons limits and the shortest path itself.
